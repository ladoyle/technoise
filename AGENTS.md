...
npm test             # vitest run — schema, publishing-rule, build-output, SEO-helper,
                      ...
                      # and tests/palette-tokens.test.ts reads tokens.css and re-derives every
                      # ratio the styleguide prints, so it also carries that page's own
                      # prose-figure assertions, not just src/lib/palette.ts's)
```
